<?php
/**
 * admin/jadwal_dokter/edit.php
 * -----------------------------------------------------------
 * Form edit jadwal dokter + proses update ke database.
 * -----------------------------------------------------------
 */
...
$judul_halaman = 'Edit Jadwal Dokter';

$error = [];
$id = $_GET['id'] ?? null;

if (!$id) {
    redirectDenganPesan('index.php', 'ID Jadwal tidak ditemukan.', 'error');
}

// Ambil data jadwal
$stmt = $koneksi->prepare("SELECT * FROM jadwal_dokter WHERE id_jadwal = ?");
$stmt->execute([$id]);
$jadwal = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cek bentrok jadwal selain jadwal ini sendiri
    if (empty($error)) {
        $stmt_cek = $koneksi->prepare("
            SELECT id_jadwal FROM jadwal_dokter 
            WHERE id_dokter = ? AND hari = ? AND id_jadwal != ?
            AND ((jam_mulai <= ? AND jam_selesai > ?) OR (jam_mulai < ? AND jam_selesai >= ?))
        ");
        $stmt_cek->execute([$id_dokter, $hari, $id, $jam_mulai, $jam_mulai, $jam_selesai, $jam_selesai]);
        if ($stmt_cek->fetch()) {
    ...
    if (empty($error)) {
        try {
            ...
            redirectDenganPesan('index.php', 'Jadwal praktik berhasil diperbarui.', 'success');
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <select name="id_dokter" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                        ...
                        <?php foreach ($daftar_dokter as $dokter): ?>
                            <?php $selected = (($_POST['id_dokter'] ?? $jadwal['id_dokter']) == $dokter['id_dokter']) ? 'selected' : ''; ?>
                ...
                <div>
                    ...
                    <select name="hari" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                        ...
                        foreach ($hari_list as $h) {
                            $selected = (($_POST['hari'] ?? $jadwal['hari']) == $h) ? 'selected' : '';
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                        <option value="aktif" <?= (($_POST['status'] ?? $jadwal['status']) === 'aktif') ? 'selected' : '' ?>>Aktif</option>
                        <option value="nonaktif" <?= (($_POST['status'] ?? $jadwal['status']) === 'nonaktif') ? 'selected' : '' ?>>Nonaktif</option>
                    </select>
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Jam Mulai <span class="text-red-500">*</span></label>
                    <input type="time" name="jam_mulai" required value="<?= clean($_POST['jam_mulai'] ?? $jadwal['jam_mulai']) ?>"
                ...
                <div>
                    ...
                    <input type="time" name="jam_selesai" required value="<?= clean($_POST['jam_selesai'] ?? $jadwal['jam_selesai']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Kuota Pasien <span class="text-red-500">*</span></label>
                    <input type="number" name="kuota_pasien" required min="1" value="<?= clean($_POST['kuota_pasien'] ?? $jadwal['kuota_pasien']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>
