<?php
/**
 * admin/data_obat/edit.php
 * -----------------------------------------------------------
 * Form edit data obat + proses update ke database.
 * -----------------------------------------------------------
 */
...
$judul_halaman = 'Edit Obat';

$error = [];
$id = $_GET['id'] ?? null;

if (!$id) {
    redirectDenganPesan('index.php', 'ID Obat tidak ditemukan.', 'error');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cek duplikasi nama obat pada ID yang berbeda
    if (empty($error)) {
        $stmt_cek = $koneksi->prepare("SELECT id_obat FROM obat WHERE nama_obat = ? AND id_obat != ?");
        $stmt_cek->execute([$nama_obat, $id]);
        if ($stmt_cek->fetch()) {
    ...
    if (empty($error)) {
        try {
            ...
            redirectDenganPesan('index.php', 'Data obat "' . $nama_obat . '" berhasil diperbarui.', 'success');
        ...
        <div class="p-6 space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="nama_obat" required value="<?= clean($_POST['nama_obat'] ?? $obat['nama_obat']) ?>"
                ...
                <div>
                    ...
                    <input type="text" name="jenis_obat" value="<?= clean($_POST['jenis_obat'] ?? $obat['jenis_obat']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Satuan</label>
                    <input type="text" name="satuan" value="<?= clean($_POST['satuan'] ?? $obat['satuan']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Tanggal Kadaluarsa</label>
                    <input type="date" name="kadaluarsa" value="<?= clean($_POST['kadaluarsa'] ?? $obat['kadaluarsa']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Stok Saat Ini <span class="text-red-500">*</span></label>
                    <input type="number" name="stok" required min="0" value="<?= clean($_POST['stok'] ?? $obat['stok']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Stok Minimum (Peringatan)</label>
                    <input type="number" name="stok_minimum" min="0" value="<?= clean($_POST['stok_minimum'] ?? $obat['stok_minimum']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>

                <div class="sm:col-span-2">
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Harga Obat (Rp)</label>
                    <input type="number" name="harga" min="0" value="<?= clean($_POST['harga'] ?? $obat['harga']) ?>"
                           class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                </div>
            </div>
        </div>
