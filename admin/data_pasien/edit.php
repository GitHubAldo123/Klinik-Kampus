<?php
/**
 * admin/data_pasien/edit.php
 * -----------------------------------------------------------
 * Form edit data pasien + proses update ke database.
 * -----------------------------------------------------------
 */
...
$judul_halaman = 'Edit Pasien';

$error = [];
$id = $_GET['id'] ?? null;

if (!$id) {
    redirectDenganPesan('index.php', 'ID Pasien tidak ditemukan.', 'error');
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Cek apakah NIM/NIK sudah terdaftar di data lain (kecuali data ini sendiri)
    if (empty($error)) {
        $stmt_cek = $koneksi->prepare("SELECT id_pasien FROM pasien WHERE nim_nik = ? AND id_pasien != ?");
        $stmt_cek->execute([$nim_nik, $id]);
        if ($stmt_cek->fetch()) {
            $error[] = 'NIM/NIK ini sudah terdaftar di sistem pada pasien lain.';
        }
    }

    // Jika lolos validasi, update database
    if (empty($error)) {
        ...
        redirectDenganPesan('index.php', 'Data pasien "' . $nama_lengkap . '" berhasil diperbarui.', 'success');
    }
}
    ...
    <form method="POST" class="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="nim_nik" required value="<?= clean($_POST['nim_nik'] ?? $pasien['nim_nik']) ?>"
            ...
            <div>
                ...
                <input type="text" name="nama_lengkap" required value="<?= clean($_POST['nama_lengkap'] ?? $pasien['nama_lengkap']) ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            </div>
            ...
            <div>
                ...
                <select name="jenis_kelamin" required class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    ...
                    <option value="Laki-laki" <?= (($_POST['jenis_kelamin'] ?? $pasien['jenis_kelamin']) === 'Laki-laki') ? 'selected' : '' ?>>Laki-laki</option>
                    <option value="Perempuan" <?= (($_POST['jenis_kelamin'] ?? $pasien['jenis_kelamin']) === 'Perempuan') ? 'selected' : '' ?>>Perempuan</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="date" name="tanggal_lahir" value="<?= clean($_POST['tanggal_lahir'] ?? $pasien['tanggal_lahir']) ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            </div>
            ...
            <div>
                ...
                <select name="status_akademik" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
                    <option value="Mahasiswa" <?= (($_POST['status_akademik'] ?? $pasien['status_akademik']) === 'Mahasiswa') ? 'selected' : '' ?>>Mahasiswa</option>
                    <option value="Dosen" <?= (($_POST['status_akademik'] ?? $pasien['status_akademik']) === 'Dosen') ? 'selected' : '' ?>>Dosen</option>
                    <option value="Tenaga Kependidikan" <?= (($_POST['status_akademik'] ?? $pasien['status_akademik']) === 'Tenaga Kependidikan') ? 'selected' : '' ?>>Tenaga Kependidikan</option>
                </select>
            </div>
            ...
            <div>
                ...
                <input type="text" name="fakultas" value="<?= clean($_POST['fakultas'] ?? $pasien['fakultas']) ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            </div>
            ...
            <div>
                ...
                <input type="text" name="no_telepon" value="<?= clean($_POST['no_telepon'] ?? $pasien['no_telepon']) ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            </div>
            ...
            <div>
                ...
                <input type="email" name="email" value="<?= clean($_POST['email'] ?? $pasien['email']) ?>"
                       class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none">
            </div>
        </div>
        ...
        <div>
            ...
            <textarea name="alamat" rows="3"
                      class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none"><?= clean($_POST['alamat'] ?? $pasien['alamat']) ?></textarea>
        </div>
